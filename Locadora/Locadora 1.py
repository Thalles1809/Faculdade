...
Original file is located at
    https://colab.research.google.com/drive/1WS3k3ro5Kdipxt5lgbRtOPtOz5jLpHFv
"""

class Rental(object):
#Classe pai que contém as listas que armazenam os valores dos objetos gerados pela execução

#Método que inicia primeiro na locadora, e que defineas listas
    def __init__(self):
        self.ClientList = []
        self.CarList = []
        self.RentList = []
...
#Método que realiza a locação dos veículos apartir das informações entregues nas outras classes,
#Trás uma função que virifica se o cpf digitado já existe e permite o cadastro de um novo cliente
#Trás uma função que verifica se existe carro cadastrado, e ultiliza como fator de busca a placa para locação
...
#Função que chama o menu do sistema, e pela função digitada faz a chamada das demais funções
    def menu(self):
...
#Método que inicia a classe, trazendo junto os dados de cada veículo que deve ser cadastrados
...
#Dunção que busca o carro que se pretende alugar
        ...
        if len(self.CarList) > 0:
            ...
            for car in self.CarList:
                if car['placa'] == self.placa:
                    ...
                    print(f"Modelo: {car['modelo']}")
                    print(f"Aluguel:{car['aluguel']}")
                    self.data_ininio = str(input("Qual a data de início da locação: "))
                    self.data_fim = str(input("Qual a data final de locação: "))
...
#Função para cadastrar os valores possíveis de categorias pré cadastradas
    def category(self):
        while True:
...
#Função para cadastrar os valores possíveis de transmissão pré cadastradas
...
#Função para cadastrar os valores possíveis de combustível pré cadastradas
    def fuel(self):
        while True:
            print(
                "Escolha uma opção\n1 - Gasolina\n2 - Álcool\n3 - Flex\n4 - Diesel\n5 - GNV")
...
#Função que recebe os atributos dos veiculos e insere dentro de um dicionário, que é inserido em uma lista
    def new_car(self):
...
class Client(Rental):
#Classe filha Client da classe pai Locadora, com os métodos dos clientes
...
#Função que altera os dados dos clientes pré-cadastrados no sistema
...
#Função que trás um relatório de cleintes já cadastrados
...
#Função que verifica se o cpf já foi ultiliado anteriormente
...
#Função que faz o cadastro de um novo cliente, e insere os dados em um dicionário, posteriormente em uma lista
    def new_client(self):
        while True:
